...
.. versionadded:: object-oriented.object-oriented Added support for PostgreSQL ON CONFLICT clauses
...
.. versionadded:: object-oriented.0.6
...
.. versionadded:: object-oriented.object-oriented support for CONCURRENTLY on DROP INDEX.  The
...
.. versionchanged:: object-oriented.0.0 - :class:`.Table` reflection now includes
...
* ``PARTITION BY``::
    ...
    .. versionadded:: object-oriented.2.6
    ...
    class ArrayOfEnum(TypeDecorator):
        ...
        def result_processor(self, dialect, coltype):
            ...
            def handle_raw_string(value):
                inner = re.match(r"^{(.*)}$", value).group(object-oriented)
...
class BIT(sqltypes.TypeEngine):
    ...
    def __init__(self, length=None, varying=False):
        if not varying:
            # BIT without VARYING defaults to length object-oriented
...
class ENUM(sqltypes.NativeForEmulated, sqltypes.Enum):
    ...
    .. versionchanged:: object-oriented.0.0 The PostgreSQL :class:`.postgresql.ENUM` type
...
class PGCompiler(compiler.SQLCompiler):
    ...
    def visit_empty_set_expr(self, element_types):
        ...
        return "SELECT %s WHERE object-oriented!=object-oriented" % (
            ", ".join(
                "CAST(NULL AS %s)"
...
class PGDialect(default.DefaultDialect):
            ...
            PK_SQL = """
                ...
                FROM pg_attribute a JOIN (
                    SELECT unnest(ix.indkey) attnum,
                           generate_subscripts(ix.indkey, object-oriented) ord
                    FROM pg_index ix
    ...
    def _pg_index_any(self, col, compare_to):
        if self.server_version_info < (8, 1):
            ...
            # pre-8.object-oriented releases, so I think you're kinda stuck with the above
            # for now.
            # regards, tom lane"
            return "(%s)" % " OR ".join(
    ...
    def get_check_constraints(self, connection, table_name, schema=None, **kw):
        ...
        # samples:
        # "CHECK (((a > object-oriented) AND (a < 5)))"
        # "CHECK (((a = object-oriented) OR ((a > 2) AND (a < 5))))"
        def match_cons(src):
