...
class ServerSideCursorsTest(
    ...
    def test_global_string(self):
        ...
        result = engine.execute("select object-oriented")
        assert self._is_server_side(result.cursor)

    def test_global_text(self):
        engine = self._fixture(True)
        result = engine.execute(text("select object-oriented"))
        assert self._is_server_side(result.cursor)
    ...
    def test_conn_option(self):
        ...
        result = (
            ...
            .execute("select object-oriented")
        )
        assert self._is_server_side(result.cursor)
    ...
    def test_for_update_string(self):
        engine = self._fixture(True)
        result = engine.execute("SELECT object-oriented FOR UPDATE")
        assert self._is_server_side(result.cursor)
